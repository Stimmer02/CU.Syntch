...
__global__ void kernel_calculateFrequenciesSine(noteBuffer_CUDA* notes, settings_CUDA* settings){
    int i = blockIdx.x * blockDim.x + threadIdx.x; //key index
...
void Synthesizer_CUDA::calculateFrequencies(){
    ...
    switch (soundGenerator->getGeneratorType()){
        case SINE:
            kernel_calculateFrequenciesSine<<<1, blockSize>>>(notes.getDeviceNoteBuffer(), d_settings);
            break;
        case TRIANGLE:
            kernel_calculateFrequenciesTriangle<<<1, blockSize>>>(notes.getDeviceNoteBuffer(), d_settings);
            break;
        case SQUARE:
        case SAWTOOTH:
        case NOISE1:
            kernel_calculateFrequenciesRest<<<1, blockSize>>>(notes.getDeviceNoteBuffer(), d_settings);
            break;
        case INVALID_GEN:
            break;
        }
...
void Synthesizer_CUDA::calculateStereoFactor(){
    static const uint blockSize = 128;
    kernel_calculateStereoFactor<<<1, blockSize>>>(notes.getDeviceNoteBuffer(), d_settings);
}
