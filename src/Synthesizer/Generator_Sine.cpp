...
void synthesizer::Generator_Sine::generate(noteBuffer& noteBuffer, const uchar* keyState, const settings& settings){
    ...
    for (; i < settings.sampleSize; i++){
        if (keyState[i]){
            ...
            noteBuffer.buffer[i] = sin((i+noteBuffer.phaze)*noteBuffer.multiplier) * settings.volume * (1-noteBuffer.lastAttack);
            ...
            noteBuffer.buffer[i] = sin((i+noteBuffer.phaze)*noteBuffer.multiplier) * settings.volume * (1-noteBuffer.lastAttack) * release;
