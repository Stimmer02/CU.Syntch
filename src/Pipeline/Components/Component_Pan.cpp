...
void Component_Pan::apply(pipelineAudioBuffer* buffer){
    for (uint i = 0; i < audioInfo->sampleSize; i++){
        buffer->bufferR[i] = settings.values[0] >= 0.5 ? buffer->bufferR[i] : buffer->bufferR[i]*settings.values[0]*2;
        buffer->bufferL[i] = settings.values[0] <= 0.5 ? buffer->bufferL[i] : buffer->bufferL[i]*(2-settings.values[0]*2);
    }
}
